...
//        assertXMLEqual(new String(out.toByteArray()), StaticTestRecords.RESOURCES_SUMMERLAND_XML);
    ...
    public void testMarcXmlWriterNormalized() throws Exception {
        ...
        final InputStream input = Thread.currentThread().getContextClassLoader().getResourceAsStream(StaticTestRecords.RESOURCES_BRKRTEST_MRC);
        assertNotNull(input);
        final MarcXmlWriter writer = new MarcXmlWriter(out, true);
    ...
    public void testOutputToDOMResult() throws Exception {
        final InputStream input = Thread.currentThread().getContextClassLoader().getResourceAsStream(StaticTestRecords.RESOURCES_SUMMERLAND_MRC);
