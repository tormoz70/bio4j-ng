...
public class AnselToUnicode extends CharConverter {
    ...
    private CodeTableInterface loadGeneratedTable(final boolean loadMultibyte) {
        try {
            ...
            if (loadMultibyte) {
                ct = new CodeTable(Thread.currentThread().getContextClassLoader()
                        .getResourceAsStream("codetables.xml"));
            } else {
                ct = new CodeTable(Thread.currentThread().getContextClassLoader()
                        .getResourceAsStream("codetablesnocjk.xml"));
            }
...
//            }
    ...
    private void loadMultibyte() {
        ct = new CodeTable(Thread.currentThread().getContextClassLoader().getResourceAsStream("codetables.xml"));
    }
