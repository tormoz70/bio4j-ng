...
//    @SuppressWarnings({"unchecked", "deprecation"})
    protected CloseableHttpClient createHttpClient(HttpParams hcParams) {
//        try {
...
//    }
    ...
    protected void service(HttpServletRequest servletRequest, HttpServletResponse servletResponse)
        ...
        if(method.equals("POST") && parts != null) {
            ...
            for(Part p : parts) {
                ...
                if(ct.equals(ContentType.TEXT_PLAIN.getMimeType())) {
                    ...
                    //String tmpFileName = java.util.UUID.randomUUID().toString() + "." + Utl.fileNameExt(fileName);
        ...
        } else if (servletRequest.getHeader(HttpHeaders.CONTENT_LENGTH) != null ||
        ...
        try {
            ...
            if(proxyRequest instanceof HttpPost)
                proxyResponse = proxyClient.execute((HttpPost)proxyRequest);
            else
    ...
    protected String rewriteQueryStringFromRequest(HttpServletRequest servletRequest, String queryString) {
        Map<String, String[]> parMap = servletRequest.getParameterMap();
        if(queryString.startsWith("?"))
            queryString = queryString.substring(1);
        String[] params = Strings.split(queryString, "&");
        queryString = null;
        for(String parPair : params){
            String[] pair = Strings.split(parPair, "=");
            if(pair.length == 2) {
                if (parMap.containsKey(pair[0]))
                    queryString = Strings.append(queryString, pair[0] + "=" + parMap.get(pair[0])[0], "&");
            }
        }
        return queryString;
    }
