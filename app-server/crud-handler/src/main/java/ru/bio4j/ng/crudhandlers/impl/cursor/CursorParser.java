...
public class CursorParser {
    ...
    private static void parsParams(final Cursor cursor) {
    ...
    private static void parsCols(final Cursor cursor) {
    ...
    private static Cursor.Type parsType(String hint) {
        return Cursor.Type.valueOf(hint.toUpperCase());
    }
    private static byte parsWrapMode(String hint) {
        ...
        if(hint.startsWith(wrapPrefix)){
            ...
            for(String flag : flags){
                Cursor.WrapMode mode = Cursor.WrapMode.NONE;
                try {
                    mode = Cursor.WrapMode.valueOf(flag.toUpperCase());
                } catch (IllegalArgumentException e) {}
                if(mode == Cursor.WrapMode.ALL) {
                    rslt = Cursor.WrapMode.ALL.code();
                    break;
                } else
                    rslt += mode.code();
            }
    ...
    private static void parsHints(final Cursor cursor) {
    ...
    public static Cursor pars(final String bioCode, final String sql) {
        Cursor cursor = new Cursor(bioCode, sql);
        parsParams(cursor);
        parsCols(cursor);
        parsHints(cursor);
        return cursor;
