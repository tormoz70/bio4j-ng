...
import ru.bio4j.ng.database.doa.impl.oracle.OraSelectableParamSetter;
import ru.bio4j.ng.database.doa.impl.wrappers.WrappersImpl;

import javax.sql.DataSource;

public class DbContext implements SQLContext {
    ...
    private DbContext(DataSource cpool, SQLConnectionPoolConfig config) throws Exception {
        this.wrappers = new WrappersImpl("oracle");
        this.cpool = cpool;
        this.config = config;
                    ...
                    new SQLConnectionConnectedEvent() {
                        ...
                        public void handle(SQLContext sender, Attributes attrs) throws SQLException {
                            if(attrs.getConnection() != null) {
                                String curSchema = DbContext.this.config.getCurrentSchema().toUpperCase();
    ...
    private void doAfterConnect(SQLConnectionConnectedEvent.Attributes attrs) throws SQLException {
    ...
    public static SQLContext create(SQLConnectionPoolConfig config) throws Exception {
        ...
        return new DbContext(dataSource, config);
    }
    ...
    public <C, R> R execBatch (final SQLAction<C, R> batch, final C context) throws Exception {
        return execBatch(this, new SQLActionExt<DbContext, C, R>() {
            @Override
            public R exec(DbContext context, Connection conn, C param) throws Exception {
                if (batch != null)
    ...
    public <C, R> R execSQLAtomic(final Connection conn, final SQLAction<C, R> action, final C context) throws Exception {
        return execSQLAtomic(this, conn, new SQLActionExt<DbContext, C, R>() {
            @Override
            public R exec(DbContext context, Connection conn, C param) throws Exception {
                if(action != null)
